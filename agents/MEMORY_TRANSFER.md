...
|------|----------------|-----------------|
| `memory.json` | `~/Trading_system/agents/memory.json` | Trained rules, cautions, win patterns, regime |
| `knowledge/` | `~/Trading_system/agents/knowledge/*.json` | Ingested external sources (Sam Seiden, ICT, etc.) |
...
cp memory_YYYYMMDD.json ~/Trading_system/agents/memory.json
...
scp ubuntu@YOUR_VPS_IP:~/Trading_system/agents/memory.json     ./memory.json
scp ubuntu@YOUR_VPS_IP:~/Trading_system/agents/knowledge/      ./knowledge/ -r
scp ubuntu@YOUR_VPS_IP:~/Trading_system/data/trades.db        ./trades.db

# Then upload to new server
scp ./memory.json   ubuntu@NEW_SERVER_IP:~/Trading_system/agents/memory.json
scp ./knowledge/    ubuntu@NEW_SERVER_IP:~/Trading_system/agents/knowledge/ -r
...
python3 agents/promote_memory.py
```
...
[ ] Copy memory.json → agents/memory.json    (Option A/B only)
[ ] Copy knowledge/ → agents/knowledge/      (Option A/B only)
[ ] Option C only: python3 agents/seed_memory.py  (creates candidate)
[ ] Option C only: python3 agents/promote_memory.py  (review + promote)
[ ] Set up cron: 09:00 brief.py, 16:00 trainer.py + promote_memory.py
[ ] Restart scheduler.py
[ ] Test: python3 agents/brief.py  (should send Telegram message)
```
...
python3 -c "
import json
m = json.load(open('agents/memory.json'))
