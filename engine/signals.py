...
def generate_signal(
    ...
    """
    Build a Signal for a zone if price is near the proximal line.
    Returns None if zone is invalid, already heavily tested, or score < MIN.

    Entry logic (Type 1 — limit at proximal, only automatable type):
      Demand: entry = proximal, SL = distal, target = entry + 2× risk (intraday)
      Supply: entry = proximal, SL = distal, target = entry − 2× risk (intraday)
    """
...
def _decide_entry_type(score: float) -> int:
    """
    Score 10   → Type 1 (limit, set and forget — automatable)
    Score 8–9  → Type 2 (enter anywhere in zone)
    Score < 8  → no trade (caller already filters this)
    """
